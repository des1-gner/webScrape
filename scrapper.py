...
def process_xml(file_paths, output_file):
    ...
    for file_path in file_paths:
        ...
        for item in root.findall('./channel/item'):
            ...
            # Format the content without the image
            formatted_content = f"Here's what I found for {title}. To get more information on {title}, go to {link} published on {pub_date}, last updated on {post_modified}."

            # Combine the content and excerpt
            combined_content = f"Content:\n{encoded}\n\nExcerpt:\n{encoded_excerpt}"

            # Write the extracted data to the sheet
            sheet.cell(row=row_num, column=1, value=combined_content)
            sheet.cell(row=row_num, column=2, value=formatted_content)
